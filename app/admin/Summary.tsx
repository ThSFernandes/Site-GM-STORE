        ...
        <div className="grid grid-cols-2 gap-3 max-h-50vh overflow-y-auto">
            {
                summaryKeys && summaryKeys.map((key) => {
                    return <div key={key} className="rounded-x1 border-2 p-4 flex flex-col items-center gap-2 transition">
                        <div className="text-x1 md:text-4x1 font-bold">
                            {
                                summaryData[key].label === 'Total das vendas' ? <>{formatPrice(summaryData[key].digit)} </> : <>{formatNumber(summaryData[key].digit)}</>
                            }
                        </div>
